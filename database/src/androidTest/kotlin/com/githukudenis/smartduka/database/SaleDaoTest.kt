...
class SaleDaoTest : BaseRoomTest() {
    ...
    fun updateSale_updatesFieldsCorrectly() = runTest {
        ...
        val updated = sale.copy(totalAmount = 750.0, paymentStatus = PaymentStatus.PAID)
    ...
    fun observeSalesForShop_ordersByDateDescending() = runTest {
        ...
        val olderSale = TestDataFactory.sale(shopId = shop.shopId, date = 1_000L)

        val newerSale = TestDataFactory.sale(shopId = shop.shopId, date = 2_000L)

        saleDao.insertSale(olderSale)
        saleDao.insertSale(newerSale)

        val sales = saleDao.observeSalesForShop(shop.shopId).first()
    ...
    fun observeSaleWithItems_returnsSaleAndItems() = runTest {
        ...
        val result = saleDao.observeSaleWithItems(sale.saleId).first()
