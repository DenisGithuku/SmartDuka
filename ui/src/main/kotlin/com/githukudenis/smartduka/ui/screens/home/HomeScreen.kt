...
fun GreetingTitle(modifier: Modifier = Modifier) {
    Column(verticalArrangement = Arrangement.spacedBy(LocalDimens.current.smallPadding)) {
        Text(
...
fun Stats(modifier: Modifier = Modifier, statItems: List<StatItem>) {
    Card(modifier = modifier) {
        Column(
            ...
            Text(text = "Total Sales", style = MaterialTheme.typography.titleSmall)
            LazyVerticalGrid(
                columns = GridCells.Fixed(2),
                verticalArrangement = Arrangement.spacedBy(LocalDimens.current.smallPadding)
            ) {
                items(statItems.size) { StatItem(statItem = statItems[it]) }
            }
        }
    }
}

data class StatItem(val title: Int, val icon: Int, val value: Double)
...
fun StatItem(statItem: StatItem, modifier: Modifier = Modifier) {
    Row(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(LocalDimens.current.extraSmallPadding)) {
            Text(text = stringResource(id = statItem.title), style = MaterialTheme.typography.labelMedium)
            Text(text = statItem.value.toString(), style = MaterialTheme.typography.titleSmall)
        }
    }
}
