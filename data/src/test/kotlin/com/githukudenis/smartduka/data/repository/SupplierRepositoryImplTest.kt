...
class SupplierRepositoryImplTest {
    ...
    fun `archiveSupplier delegates to datasource`() = runTest {
        ...
        coVerify {
            supplierLocalDataSource.archiveSupplier("supplier-1")
        }
    }
    ...
    fun `getSupplierById returns mapped domain object`() = runTest {
        ...
        coEvery {
            supplierLocalDataSource.getSupplierById(entity.supplierId)
        } returns entity
    ...
    fun `getSupplierById returns null when not found`() = runTest {
        coEvery {
            supplierLocalDataSource.getSupplierById(any())
        } returns null
    ...
    fun `observeSuppliersForShop maps entities to domain`() = runTest {
        val entities = listOf(
            TestDataFactory.supplierEntity("1"),
            TestDataFactory.supplierEntity("2")
        )

        every {
            supplierLocalDataSource.observeSuppliersByShop("shop-1")
        } returns flowOf(entities)

        repository.observeSuppliersForShop("shop-1")
            .test {
    ...
    private object TestDataFactory {
        fun supplier(id: String = "supplier-1") = Supplier(
            supplierId = id,
            shopId = "shop-1",
        ...
        fun supplierEntity(id: String = "supplier-1") = SupplierEntity(
            supplierId = id,
            shopId = "shop-1",
            name = "Default Supplier",
            contact = "0700000000",
